...
class TestRTMClient_Issue_569(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not hasattr(self, "cpu_monitor") or not TestRTMClient_Issue_569.cpu_monitor.is_alive():
            def run_cpu_monitor(self):
                self.logger.debug("Starting CPU monitor in another thread...")
                TestRTMClient_Issue_569.cpu_usage = 0
                while True:
                    p = psutil.Process(os.getpid())
                    current_cpu_usage: float = p.cpu_percent(interval=0.5)
                    self.logger.debug(current_cpu_usage)
                    if current_cpu_usage > TestRTMClient_Issue_569.cpu_usage:
                        TestRTMClient_Issue_569.cpu_usage = current_cpu_usage

            TestRTMClient_Issue_569.cpu_monitor = threading.Thread(target=run_cpu_monitor, args=[self])
            TestRTMClient_Issue_569.cpu_monitor.setDaemon(True)
            TestRTMClient_Issue_569.cpu_monitor.start()
                ...
                if not str(event["text"]).startswith("Current CPU usage:"):
                    ...
                    for i in range(0, 3):
                        new_message = web_client.chat_postMessage(
                            ...
                            text=f"Current CPU usage: {TestRTMClient_Issue_569.cpu_usage} % (test_cpu_usage)"
                        )
                ...
                if not str(event["text"]).startswith("Current CPU usage:"):
                    ...
                    for i in range(0, 3):
                        new_message = await web_client.chat_postMessage(
                            channel=event["channel"],
                            text=f"Current CPU usage: {TestRTMClient_Issue_569.cpu_usage} % (test_cpu_usage_async)"
                        )
                        self.logger.debug(new_message)
                        self.call_count += 1
