...
class TestWebClient(unittest.TestCase):
    ...
    def test_sync(self):
        ...
        channel_message = client.chat_postMessage(
            channel="#random", blocks=[{"type": "call", "call_id": call_id,}]
        )
        ...
        new_participants = client.calls_participants_add(
            id=call_id,
            users=[
                {
        ...
        participants_removal = client.calls_participants_remove(
            id=call_id,
            users=[
                {
                    "external_id": "anon-222",
    ...
    async def test_async(self):
        ...
        channel_message = await client.chat_postMessage(
            channel="#random", blocks=[{"type": "call", "call_id": call_id,}]
        )
        self.assertIsNotNone(channel_message)
        ...
        new_participants = await client.calls_participants_add(
            id=call_id,
            users=[
                {
                    "external_id": "anon-222",
