...
class TestWebClient(unittest.TestCase):
    ...
    def test_legacy_bytes_for_file_param_bytes(self):
        client = LegacyWebClient(token=self.bot_token, run_async=False)
    ...
    @async_test
    async def test_legacy_bytes_for_file_param_bytes_async(self):
        client = LegacyWebClient(token=self.bot_token, run_async=True)
        bytes = BytesIO(bytearray("This is a test (bytes)", "utf-8")).getvalue()
